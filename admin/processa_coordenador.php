...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO coordenador (nomeCoordenador, nomesocial, fkPronomeTratamento , fkPronomeReferencia, areaAtuacao, emailCoordenador, senhaCoordenador, dataCadastro) 
    ...
    if ($stmt->execute()) {
        ...
        $idCoordenador = $conexao->lastInsertId();

        // Atualiza a tabela empresa
        $updateSql = "UPDATE empresa SET fkCoordenador = :idCoordenador WHERE codigoProjetoConvenio = :codigo";
        $updateStmt = $conexao->prepare($updateSql);
        $updateStmt->bindParam(':idCoordenador', $idCoordenador);
        $updateStmt->bindParam(':codigo', $codigo);

        if($updateStmt->execute()){
            echo "Coordenador cadastrado e empresa atualizada com sucesso!";
        } else {
            echo "Coordenador cadastrado, mas erro ao atualizar a empresa.";
        }
    } else {
    ...
    header('Location: cadastro_coordenador.php');
}
?>
